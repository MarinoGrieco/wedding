...
const DB = {
  ...
  async initDefaultData() {
    const snap = await db.collection('guests').limit(1).get();
    if (!snap.empty) return; // Already has data
    const defaults = [
      { family_name: 'Rossi', invitation_code: 'ROSSI1', max_guests: 4, status: 'pending', rsvp_date: null },
      { family_name: 'Bianchi', invitation_code: 'BLANC2', max_guests: 3, status: 'pending', rsvp_date: null },
      { family_name: 'Verdi', invitation_code: 'VERDE3', max_guests: 2, status: 'pending', rsvp_date: null }
    ];
    const batch = db.batch();
    defaults.forEach(g => {
    ...
    await batch.commit();
    console.log('Default guests created');
  }
};
