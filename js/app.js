...
const App = {
  ...
  generateQR() {
    ...
    if (typeof QRCode !== 'undefined') {
      ...
      QRCode.toCanvas(canvas, url, { width: 280, margin: 2, color: { dark: '#3d2c22', light: '#ffffff' } }, (err) => {
    ...
    document.getElementById('app').innerHTML = `
      <div class="page">
        <div class="setup-message">
          ...
          <p class="text-muted mt-2">Per utilizzare il sito, devi configurare Firebase.</p>
          <p class="text-muted">Apri il file <strong>js/config.js</strong> e inserisci le tue credenziali Firebase:</p>
  ...
  ...
          ...
          <p class="text-muted">Consulta il file <strong>README.md</strong> per le istruzioni complete.</p>
        </div>
      </div>`;
  },
  ...
  async renderHome() {
    ...
    if (dateObj) {
      ...
      if (diff > 0) {
        ...
        countdownHtml = `<p style="font-size:1.2rem;margin-top:8px;color:var(--primary-dark);font-weight:600;">Mancano <strong>${days}</strong> giorni! 🎉</p>`;
      }
    }
    ...
    document.getElementById('app').innerHTML = `
      <div class="page">
        <div class="hero">
          ...
          <h1>${this.esc(info.spouse1_name)} &amp; ${this.esc(info.spouse2_name)}</h1>
          <p>${dateStr}</p>
          ${countdownHtml}
        </div>

        <div class="card">
          <h3 class="mb-2">⛪ Cerimonia</h3>
          <div class="info-row">
          ...
          <div class="info-row">
            ...
            <div class="info-content">
              <h4>Luogo</h4>
              <p>${this.esc(info.ceremony_location)}</p>
        ...
        <div class="card">
          ...
          <div class="info-row">
            ...
            <div class="info-content">
              <h4>Luogo</h4>
              <p>${this.esc(info.reception_location)}</p>
        ...
        <div class="card text-center">
          <p style="font-style:italic;color:var(--text-light);margin:0;">${this.esc(info.notes)}</p>
        </div>` : ''}

        <div class="divider">💍</div>

        <button class="btn btn-primary btn-block" data-action="navigate" data-value="/rsvp" style="font-size:1.1rem;padding:16px;">
          ...
          <form data-form="code-entry">
            <div class="form-group">
              ...
              <input type="text" name="code" class="form-input" placeholder="Es: ROSSI1"
                     ...
                     style="text-transform:uppercase;text-align:center;font-size:1.2rem;letter-spacing:2px;">
            </div>
  ...
  renderRSVPForm() {
    ...
    const cards = this.participants.map((p, i) => `
      <div class="participant-card">
        <div class="participant-header">
          ...
          ${this.participants.length > 1 ? `<button type="button" class="btn btn-sm btn-danger" data-action="remove-participant" data-value="${i}">✕</button>` : ''}
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label class="form-label">Intolleranze / Allergie alimentari</label>
          <textarea name="intol_${i}" class="form-textarea" placeholder="Es: celiaco, intollerante al lattosio..." rows="2">${this.esc(p.intolerances)}</textarea>
        </div>
      </div>`).join('');
    ...
    document.getElementById('app').innerHTML = `
      <div class="page">
        ...
        <div class="text-center mb-3">
          <h2>Famiglia ${this.esc(guest.family_name)}</h2>
        ...
        <form data-form="rsvp">
          ...
          ` : `
            <div class="card text-center">
              ...
              <p class="text-muted" style="margin:0;">Ci dispiace che non possiate venire!</p>
            </div>
          `}

          <button type="submit" class="btn btn-primary btn-block mt-2" style="font-size:1.05rem;padding:14px;">
            📤 Invia Risposta
          </button>
        </form>
      </div>`;
    ...
    document.getElementById('app').innerHTML = `
      <div class="page">
        <div class="text-center" style="padding-top:40px;">
          ...
          <h2>${ok ? 'Grazie per la conferma!' : 'Risposta registrata'}</h2>
          <p class="text-muted mt-2">${ok
            ...
            <div class="card mt-3 text-left">
              ...
              ${this.participants.map(p => `
                <div style="padding:8px 0;border-bottom:1px solid var(--border);">
                  ...
                  ${p.is_child ? ' <span class="badge badge-neutral">Bambino</span>' : ''}
  ...
  async renderAdminDashboard() {
    ...
    return `
      <div class="stats-grid">
        ...
        <div class="stat-card"><div class="stat-number">${totalParts}</div><div class="stat-label">Partecipanti</div></div>
      ...
      ${intolerances.length > 0 ? `
        ...
        </div>` : '<div class="card text-center"><p class="text-muted" style="margin:0;">Nessuna intolleranza segnalata</p></div>'}`;
  },
  ...
  async renderAdminGuests() {
      ...
      return `
        <div class="guest-item">
          ...
          <div class="guest-meta">
            <span class="guest-code" data-action="copy-code" data-value="${this.esc(g.invitation_code)}" title="Clicca per copiare">📋 ${this.esc(g.invitation_code)}</span>
            <span>👥 Max: ${g.max_guests}</span>
            <button class="btn btn-sm btn-danger" data-action="delete-guest" data-value="${g.id}" style="margin-left:auto;padding:4px 10px;min-height:32px;font-size:0.8rem;">🗑️</button>
          </div>
          ${parts.length > 0 ? `
    ...
    return `
      ...
      <div class="flex-between mb-2">
        ...
        <button class="btn btn-sm btn-secondary" data-action="export-csv">📥 CSV</button>
      </div>
      ${items || '<div class="card text-center"><p class="text-muted" style="margin:0;">Nessun invitato ancora</p></div>'}`;
  },
    ...
    return `
      <div class="card">
        ...
        <form data-form="wedding-settings">
          ...
          <div class="form-group"><label class="form-label">Note</label><textarea name="notes" class="form-textarea">${this.esc(info.notes)}</textarea></div>
    ...
    return `
      <div class="card">
        ...
        <div class="form-group">
          ...
          <input type="url" name="site_url" class="form-input" value="${baseUrl}" placeholder="https://tuosito.github.io/matrimonio/">
        </div>
